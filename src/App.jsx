...
export default function App() {
  ...
  // ジム外メニュー設定（初期値：条件付き100回以上、単位：回）
  const [squatSettings, setSquatSettings] = useState({ 
    name: '自重スクワット', 
    defaultReps: 30, 
    defaultSets: 1, 
    unit: '回',
    countCondition: 'conditional', // 'always', 'never', 'conditional'
    conditionValue: 100
  });
  ...
  // ジム外アクティビティが条件を満たしているか判定
  const isSquatConditionMet = (dateStr) => {
    const total = getDailySquatTotal(dateStr);
    if (squatSettings.countCondition === 'never') return false;
    if (squatSettings.countCondition === 'always') return total > 0;
    if (squatSettings.countCondition === 'conditional') return total >= squatSettings.conditionValue;
    return total >= 100; // default fallback
  };
  ...
  const calculatePaceInfo = () => {
    ...
    const recordDates = new Set([
      ...
      ...Object.keys(squatHistory).filter(d => d.startsWith(prefix))
    ]);
    ...
    recordDates.forEach(dateStr => {
      ...
      if (d <= currentDay) {
        ...
        // 条件を満たせば加点
        if (isSquatConditionMet(dateStr)) {
            totalScore += 10;
        }
      }
    });

    const paceValue = currentDay > 0 ? totalScore / currentDay : 0;
    ...
    } else if (paceValue >= 3.3) {
      rank = "S";
      colorClass = "bg-green-50 text-green-600";
      rankColor = "text-green-600";
    } else if (paceValue >= 2.0) { 
      rank = "A";
  ...
  const calculateLastMonthPace = () => {
    ...
    recordDates.forEach(dateStr => {
      if (history[dateStr]) totalScore += 10;
      if (isSquatConditionMet(dateStr)) totalScore += 10;
    });

    return daysInLastMonth > 0 ? (totalScore / daysInLastMonth).toFixed(1) : '0.0';
  };
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      sortedDates.forEach(dateStr => {
        ...
        const isSquatDone = isSquatConditionMet(dateStr);
        
        let content = [];
        if (type) content.push(`メニュー${type}`); 
        if (isSquatDone) content.push(squatSettings.name);
        else if (squatTotal > 0) content.push(`${squatSettings.name}(${squatTotal}${squatSettings.unit})`);
  ...
  const renderCalendar = () => {
    ...
    // カウント条件を満たしている場合のみ集計に含める
    const monthlyTotal = new Set([
      ...Object.keys(history).filter(d => d.startsWith(prefix)),
      ...Object.keys(squatHistory).filter(d => d.startsWith(prefix) && isSquatConditionMet(d))
    ]).size;
      ...
      <div ref={calendarRef} className="calendar-container bg-white rounded-2xl shadow-sm p-5 mb-6 relative z-0">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex-1 flex justify-between items-center px-2">
            ...
            <div className="text-xs font-medium text-gray-400 bg-gray-50 px-2 py-1 rounded">
              TOTAL: <span className="text-gray-700 font-bold text-sm">{monthlyTotal}</span> 回
            </div>
          </div>
        ...
        <div className="grid grid-cols-7 gap-y-2 gap-x-1">
          {days.map((day, idx) => {
            ...
            const squatTotal = getDailySquatTotal(dateStr);
            const isSquatDone = isSquatConditionMet(dateStr);
            const isSquatPartial = squatTotal > 0 && !isSquatDone;
            ...
            if (menu) {
              if (isSquatDone) {
                bgClass = "bg-white";
                borderClass = `border-2 ${menu.borderColor}`;
                Content = (
                  <div className={`w-full h-full rounded-full flex items-center justify-center ${menu.bgColor}`}>
                    <div className={`w-3/4 h-3/4 rounded-full border-2 ${menu.borderColor} ${menu.color} opacity-30`}></div>
                  </div>
                );
              } else {
                bgClass = menu.bgColor.replace('50', '100'); // 少し濃くする
              ...
              Content = <div className="w-full h-full rounded-full bg-green-500 opacity-20"></div>;
            } else if (isReserved) {
            ...
            return (
              ...
              >
                <div className={`absolute inset-1 rounded-full ${bgClass} ${borderClass}`}>
                  {Content}
                </div>
                <span className={`relative z-10 text-sm font-medium leading-none ${textClass} ${isToday ? 'underline decoration-2 decoration-gray-400 underline-offset-4' : ''}`}>
                  {day.getDate()}
                </span>
                ...
                {isSquatPartial && (
                  <div className="absolute bottom-1 right-1 z-10 w-1.5 h-1.5 rounded-full bg-green-500"></div>
                )}
              </div>
            );
          })}
        ...
        <div className="mt-6 flex justify-center items-center gap-4 flex-wrap text-[10px] text-gray-400">
            ...
            <div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-full bg-green-200"></div> {squatSettings.name}</div>
            <div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-full border border-red-400 flex items-center justify-center"><div className="w-1 h-1 bg-red-400 rounded-full"></div></div> ダブル達成</div>
        </div>
  ...
  const renderEditModal = () => {
    ...
    const isReserved = reservations[dateStr];
    const dailySquat = getDailySquatTotal(dateStr);
                ...
                <div className="border-2 rounded-2xl p-4 border-green-500 bg-green-50 bg-opacity-20">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="pt-2 border-t border-green-200">
                            <label className="text-xs text-green-600 font-bold block mb-2">今月の運動回数に含めるか</label>
                            <div className="space-y-2">
                                <label className="flex items-center gap-2 cursor-pointer">
                                    <input 
                                        type="radio" 
                                        name="countCondition"
                                        checked={localSquat.countCondition === 'always'}
                                        onChange={() => setLocalSquat({...localSquat, countCondition: 'always'})}
                                        className="text-green-500 focus:ring-green-500"
                                    />
                                    <span className="text-sm text-gray-700 font-medium">含める</span>
                                </label>
                                <label className="flex items-center gap-2 cursor-pointer">
                                    <input 
                                        type="radio" 
                                        name="countCondition"
                                        checked={localSquat.countCondition === 'never'}
                                        onChange={() => setLocalSquat({...localSquat, countCondition: 'never'})}
                                        className="text-green-500 focus:ring-green-500"
                                    />
                                    <span className="text-sm text-gray-700 font-medium">含めない</span>
                                </label>
                                <label className="flex items-center gap-2 cursor-pointer">
                                    <input 
                                        type="radio" 
                                        name="countCondition"
                                        checked={localSquat.countCondition === 'conditional'}
                                        onChange={() => setLocalSquat({...localSquat, countCondition: 'conditional'})}
                                        className="text-green-500 focus:ring-green-500"
                                    />
                                    <span className="text-sm text-gray-700 font-medium">条件付きで含める</span>
                                </label>
                            </div>
                            
                            {localSquat.countCondition === 'conditional' && (
                                <div className="mt-2 flex items-center gap-2 bg-white p-2 rounded-lg border border-green-100">
                                    ...
                                    <input 
                                        type="number"
                                        value={localSquat.conditionValue}
                                        onChange={(e) => setLocalSquat({...localSquat, conditionValue: Number(e.target.value)})}
                                        className="w-20 border-b border-green-300 text-center font-bold text-green-700 outline-none"
                                    />
                                    <span className="text-xs text-gray-500">{localSquat.unit}以上ならカウント</span>
                                </div>
                            )}
                        </div>
                    </div>
              ...
              {menu.items.map((item, itemIndex) => {
                ...
                return (
                  <div key={itemIndex} className={`p-4 rounded-2xl border-2 transition-all duration-300 ${isItemComplete ? 'border-green-400 bg-green-50/50' : 'border-gray-100 bg-white'}`}>
                    ...
                    {/* セットボタンの間隔を広げ、サイズを大きく修正 (gap-6, w-14, h-14) */}
  ...
  if (currentView === 'squat') {
    return (
      <div className="min-h-screen bg-gray-50 p-4 font-sans">
        ...
        <div className="bg-white rounded-3xl p-6 shadow-sm border-t-4 border-green-500 mb-6">
          <div className="flex items-center justify-between mb-4">
            <div><h2 className="text-2xl font-black text-green-700">今日の積み上げ</h2><p className="text-xs text-gray-400 mt-1">スキマ時間で習慣化！</p></div>
            <div className="text-right"><span className={`text-4xl font-black tracking-tight ${getDailySquatTotal(getTodayString()) >= 100 ? 'text-green-500' : 'text-gray-800'}`}>{getDailySquatTotal(getTodayString())}</span><span className="text-sm text-gray-400 font-bold"> / {squatSettings.conditionValue > 0 ? squatSettings.conditionValue : 100}</span></div>
          </div>
          <div className="w-full bg-gray-100 rounded-full h-3 mb-8 overflow-hidden"><div className="bg-green-500 h-full rounded-full transition-all duration-1000 ease-out" style={{ width: `${Math.min(100, (getDailySquatTotal(getTodayString()) / (squatSettings.conditionValue > 0 ? squatSettings.conditionValue : 100)) * 100)}%` }}></div></div>
          
          <div className="bg-green-50 rounded-2xl p-5 mb-4">
            <div className="flex items-center gap-2 mb-3"><Zap size={18} className="text-green-600"/><span className="font-bold text-green-800 text-sm">記録を追加</span></div>
            <div className="flex gap-2 items-end">
                ...
                <button onClick={() => addSquatRecord(getTodayString())} className="bg-green-600 text-white p-4 rounded-xl shadow-lg shadow-green-200 hover:bg-green-700 active:scale-95 transition-all"><CalendarIcon size={24} /></button>
            </div>
          </div>
          
          <div className="space-y-2">
            {(squatHistory[getTodayString()] || []).length === 0 && <p className="text-center text-gray-300 py-8 text-sm font-medium">今日はまだ記録がありません</p>}
            {(squatHistory[getTodayString()] || []).map((record) => (
                <div key={record.id} className="flex justify-between items-center bg-gray-50 px-4 py-3 rounded-xl border border-gray-100 group">
                    ...
                    <div className="flex items-center gap-4">
                      <span className="text-xs font-bold text-green-600 bg-green-100 px-2 py-1 rounded-md">= {record.reps * record.sets}{squatSettings.unit}</span>
        ...
        <button onClick={() => setCurrentView('home')} className="w-full py-4 text-gray-500 font-bold text-sm hover:bg-gray-100 rounded-xl transition-all">ホームに戻る</button>
      </div>
    );
  }
  ...
  const isTodaySquatDone = isSquatConditionMet(todayStr);
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 font-sans max-w-md mx-auto">
            ...
            <div className="flex flex-col items-end gap-1">
                <div className="flex gap-2">
                    <div className="text-[10px] text-gray-400 font-medium text-right">
                        先月: <span className="font-bold text-gray-500">{lastMonthPace}</span>
                    </div>
      ...
      <div className="mb-8 space-y-4">
        ...
        {isTodaySquatDone && (
            <div className="bg-gradient-to-r from-yellow-400 to-orange-500 rounded-2xl p-4 flex items-center gap-4 text-white shadow-lg shadow-orange-200 animate-in fade-in slide-in-from-top-2">
                <div className="bg-white/20 p-2 rounded-full"><Trophy size={24} className="text-white" /></div>
                <div><p className="font-bold text-lg">{squatSettings.name} 目標達成！</p><p className="text-orange-100 text-xs">素晴らしい継続力です</p></div>
            </div>
        )}
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="text-left">
                        ...
                        <div className="text-xs text-gray-400 flex items-center gap-2 mt-0.5">
                            <span>スキマ時間で習慣化！</span>
                            {todaySquat > 0 && <span className="font-bold text-green-600 bg-green-50 px-2 py-0.5 rounded text-[10px]">Today: {todaySquat}{squatSettings.unit}</span>}
                        </div>
                    </div>
                </div>
